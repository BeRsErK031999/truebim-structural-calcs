...
| round column | Public punching shear tools commonly support non-rectangular columns. | Round center draft DTO, circular perimeter approximation, SVG/report exposure, diagnostics, and verification template exist. Round edge/corner are not implemented. | draft | Collect trusted SP63 round-column examples, verify perimeter/stress/resistance rules, and add expected values before promotion. | P1 |
...
| shear reinforcement | Public punching shear tools commonly include transverse reinforcement input and capacity contribution. | Draft DTO, steel class placeholders, draft area/contribution/capacity/utilization, SVG/report exposure, diagnostics, and verification templates exist. | draft | Verify SP63 reinforcement rules, steel values, layout assumptions, and trusted expected values before promotion. | P0 |
