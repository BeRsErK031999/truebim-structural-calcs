...
import type { VerificationSnapshot } from './verificationSnapshot'

export function buildVerificationSnapshotHtml(snapshot: VerificationSnapshot) {
...
<body>
  <main>
    ...
    ${renderTable([
      ...
      ['boundary classification', snapshot.metadata.boundaryClassification],
      ['geometry draft-ready', String(snapshot.metadata.geometryDraftReady)],
      ['stress draft-ready', String(snapshot.metadata.stressDraftReady)],
      ['verified arithmetic available', String(snapshot.metadata.verifiedArithmeticAvailable)],
    ])}

    <h2>Geometry Verification</h2>
    ${renderTable([
      ['active perimeter', `${result.perimeter.perimeterMm} mm`],
      ['clipped perimeter', `${result.perimeter.clippedPerimeterMm} mm`],
      ['removed perimeter', `${result.perimeter.removedPerimeterMm} mm`],
    ...
    <h2>Diff Summary</h2>
    <ul>${snapshot.geometryComparison.diffSummary.map((item) => `<li>${escapeHtml(item)}</li>`).join('')}</ul>

    <h2>SVG Verification View</h2>
    ${renderSvg(snapshot)}
  </main>
</body>
</html>`
...
function renderSvg(snapshot: VerificationSnapshot) {
...
function renderSvgElement(element: SvgSketchElement): string {
  ...
  if (element.type === 'line') {
    const dashArray = element.role === 'removed-perimeter' || element.role === 'opening-tangent' ? '8 6' : '0'
    const width = element.role === 'stress-segment' ? 7 : 2

    return `<line x1="${element.start.x}" y1="${element.start.y}" x2="${element.end.x}" y2="${element.end.y}" stroke="${stroke}" stroke-width="${width}" stroke-dasharray="${dashArray}" vector-effect="non-scaling-stroke" />`
  }
